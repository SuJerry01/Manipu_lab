...
FRANKA_PANDA_LIBERO_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{ISAACLAB_NUCLEUS_DIR}/Robots/FrankaEmika/panda_instanceable.usd",
        activate_contact_sensors=True,  # for contact sensor
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            max_depenetration_velocity=5.0,
        ),
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        ...
        pos=(-0.51, 0.0, 0.42),  # for libero_living_room_tabletop_manipulation task
    ),
    actuators={
        "panda_shoulder": ImplicitActuatorCfg(
            joint_names_expr=["panda_joint[1-4]"],
        ...
        "panda_hand": ImplicitActuatorCfg(
            joint_names_expr=["panda_finger_joint.*"],
            effort_limit_sim=10.0,  # decrease effort limit for not penetrating the object
            velocity_limit_sim=0.2,
            stiffness=2e3,
            damping=1e2,
            friction=1.5,  # increase friction for grasping
        ),
    },
    soft_joint_pos_limit_factor=1.0,
)

FRANKA_PANDA_LIBERO_HIGH_PD_CFG = FRANKA_PANDA_LIBERO_CFG.copy()
...
"""
This is the config for the franka_2f_85 gripper the same as official USD recommended
"""
FRANKA_ROBOTIQ_GRIPPER_CFG = ArticulationCfg(
    ...
    actuators={
        ...
        "gripper_drive": ImplicitActuatorCfg(
            joint_names_expr=["finger_joint"],  ##"right_outer_knuckle_joint" is the mimic_joint
...
"""
This is the config for the franka_2f_85 gripper the same as customer x-humanoid provided
"""
FRANKA_ROBOTIQ_GRIPPER_CFG_XHUMANOID = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{ISAACLAB_ASSETS_DATA_DIR}/Robots/Franka/franka_2f_85_xhumanoid.usd",
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=True,
            max_depenetration_velocity=5.0,
    ...
    actuators={
        ...
        # set PD to zero for passive joints in close-loop gripper
        "gripper_passive": ImplicitActuatorCfg(
            joint_names_expr=[".*_inner_finger_joint", ".*_outer_finger_joint", "right_outer_knuckle_joint"],
            effort_limit_sim=1.0,
            velocity_limit_sim=10.0,
            stiffness=0.0,
            damping=0.0,
