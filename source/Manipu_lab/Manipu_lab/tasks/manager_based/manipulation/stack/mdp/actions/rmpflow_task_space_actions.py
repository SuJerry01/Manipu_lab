...
class RMPFlowAction(ActionTerm):
    ...
    def action_dim(self) -> int:
        ...
        # self._rmpflow_controller.num_actions = 7 since it use quaternions (w,x,y,z) for command
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # self.ee_euler_des = torch.stack(math_utils.euler_xyz_from_quat(self.ee_quat_des), dim=-1)
        self.ee_pose_des = torch.cat([self.ee_pos_des, self.ee_quat_des], dim=1)  # shape: [n, 7]
        # print(f"\n actions: {self._processed_actions}")
        # print(f"set_command: {self.ee_pose_des}")
        # set command into controller
        self._rmpflow_controller.set_command(self.ee_pose_des)

    ## This is called each simulationcontext.step(), step 3 times when env.step() update actions
    def apply_actions(self):
        ...
        # print(f"apply_actions: {joint_pos_des} | {joint_vel_des}")
        # print(f"\njoint_pos: {joint_pos}")
