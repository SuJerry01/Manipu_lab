...
class OperationalSpaceControllerAction(ActionTerm):
    ...
    def _preprocess_actions(self, actions: torch.Tensor):
        ...
        if self._pose_rel_idx is not None:
            ...
            """
            # -- libero orientation filter -- #
            # We only want to update goal orientation if there is a valid delta ori value OR if we're using absolute ori
            # Check if there are any non-zero delta orientation values
            delta_ori = self._processed_actions[:, self._pose_rel_idx + 3 : self._pose_rel_idx + 6]
            has_valid_delta_ori = torch.any(torch.abs(delta_ori) > 1e-6, dim=-1)  # Check if any component is non-zero

            # Only apply orientation scaling if there are valid delta orientation values
            # or if we're using absolute orientation control
            valid_ori_mask = has_valid_delta_ori | (self._pose_abs_idx is not None)
            self._processed_actions[:, self._pose_rel_idx + 3 : self._pose_rel_idx + 6] *= valid_ori_mask.unsqueeze(-1) * self._orientation_scale
            """
