...
class AbsGripperJointPositionAction(JointAction):
    """
    Binary joint action that sets the binary action into joint position targets.
      - define a new process_actions to convert input actions to binary actions
    """

    cfg: actions_cfg.AbsGripperJointPositionActionCfg
    """The configuration of the action term."""
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # apply the affine transformations
        scaled_and_offset = self._raw_actions[:, 0] * self._scale + self._offset
        self._processed_actions[:, 0] = scaled_and_offset
        self._processed_actions[:, 1] = -scaled_and_offset
        self._processed_actions[:, 2] = -scaled_and_offset
        # clip actions
        if self.cfg.clip is not None:
            self._processed_actions = torch.clamp(
                self._processed_actions, min=self._clip[:, :, 0], max=self._clip[:, :, 1]
