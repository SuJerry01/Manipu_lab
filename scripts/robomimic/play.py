...
# parser.add_argument("--checkpoint", type=str, default="./logs/robomimic/Isaac-Stack-Cube-Franka-IK-Rel-v0/bc/20250207140742/models/model_epoch_2000.pth", help="Pytorch model checkpoint to load.")
parser.add_argument(
    "--task", type=str, default="Isaac-Stack-Cube-UR10-Short-Suction-IK-Rel-v0", help="Name of the task."
)
parser.add_argument(
    "--checkpoint",
    type=str,
    default="./logs/robomimic/Isaac-Stack-Cube-UR10-Short-Suction-IK-Rel-v0/bc_rnn/20250306100245/models/model_epoch_2000.pth",
...
if "Suction" in args_cli.task or "Galbot" in args_cli.task:
...
def rollout(policy, env, success_term, horizon, device, success_term):
    ...
    for i in tqdm(range(horizon)):
        ...
        for ob in obs:
            obs[ob] = torch.squeeze(
                obs[ob], dim=0
            )  # for suction_cup only has one action, need to specify 'dim=0', otherwise, will squeeze all dimensions.
        ...
        # PLAYGOUND: actions = torch.from_numpy(actions).to(device=device).view(1, -1)

        # Apply actions

        # operate suction cup ##
        if "Suction" in env.unwrapped.cfg.env_name:
            if actions[0, -1] > 0:
                env.open_suction_cup(0)
            else:
                env.close_suction_cup(0)
            # Apply actions
            obs_dict, _, terminated, truncated, _ = env.step(actions[:, :-1])
        else:
            # Apply actions
...
def main():
    ...
    for trial in range(args_cli.num_rollouts):
        ...
        t0 = time.time()
        terminated, traj = rollout(policy, env, args_cli.horizon, device, success_term)
        results.append(terminated)

        if terminated:
            time_costs.append(time.time() - t0)
            trajectory_lengths.append(len(traj["actions"]))
        ...
        print(f"Trajectory lengths mean: {np.mean(trajectory_lengths)}\n")
        print(f"Time costs mean: {np.mean(time_costs)}\n")
    env.close()
